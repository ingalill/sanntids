...
public class ArduinoDriver extends Thread{
    ...
    private final String cLeft = "a50 "; //Turn left 
    ...
    private final String cRightSlow="r30 "; //Set right wheels to standard speed
    private final String cLeftFast="l120 "; //Set left wheels to full speed
    private final String cLeftSlow="l30 "; //Set left wheels to standard speed
    private final String cTurnFast="r90 "; //Set left wheels to full speed
    private final String cTurnSlow="l0 "; //Set left wheels to standard speed
    ...
    public void seek(){
       ...
       communication.sendCommand(cTurnFast);
//        communication.sendCommand(cTurnSlow);
...
//            while((objectCaught)&&(commandBox.isGoalFound())&&(commandBox.isAutoDrive())&&(!nearGoal))
           while((commandBox.isGoalFound())&&(commandBox.isAutoDrive())&&(!nearGoal)){
                ...
                if(distanceFromGoal<50) nearGoal = true;
...
//           } else if((commandBox.isGoalFound())&&(objectCaught)&&(commandBox.isAutoDrive())){
            if((commandBox.isGoalFound())&&(commandBox.isAutoDrive())){
               goalReached=true;
               System.out.println("goal reached");
               communication.sendCommand(cBackward);
               while((true)&&(commandBox.isAutoDrive())){
                    distanceFromGoal=commandBox.getGoalDistance();
                    if(distanceFromGoal>100) break;
               }
               
           }
           if(!commandBox.isGoalFound()){
                    locateGoal();
                }
        }
        objectCaught=false;
